...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        // Создаем карту для записи количества вхождений каждого символа.
        Map<Character, Integer> count = new HashMap<>();
        for (char c : s.toCharArray()) {
            count.put(c, count.getOrDefault(c, 0) + 1);
        }

        // 2. Создаем приоритетную очередь (min-heap) для формирования дерева Хаффмана.
        PriorityQueue<Node> priorityQueue = new PriorityQueue<>();
        for (Map.Entry<Character, Integer> entry : count.entrySet()) {
            // Каждый лист – это узел с частотой появления символа.
            priorityQueue.add(new LeafNode(entry.getValue(), entry.getKey()));
        }

        // 3. Построение дерева Хаффмана.
        // Если в строке всего один уникальный символ,
        // назначим ему код "0" (специальный случай).
        if (priorityQueue.size() == 1) {
            Node onlyNode = priorityQueue.poll();
            onlyNode.fillCodes("0");
        } else {
            // Пока в очереди больше одного узла,
            // извлекаем два наименьших и объединяем их в новый узел.
            ...
            // Корень дерева будет единственным элементом в очереди.
            Node root = priorityQueue.poll();
            root.fillCodes("");
        }

        // 4. Формирование закодированной строки.
        // Для каждого символа исходной строки получаем соответствующий код из заранее заполненной карты.
